...
- `ofw-intent` — a closed POSIX tokenizer that refuses any command it cannot reduce to literal words, plus per-subcommand flag allowlists for a read-only Git subset (`status`, `rev-parse`). Each subcommand states its own effect, privilege and publication risk in the grammar table beside its allowlist, so a subcommand added later cannot inherit the read-only subset's answers by default. The execution surface is the one property no subcommand may declare itself exempt from.
